---
description: Understand the previous agent's context and load files from a context bundle with their original read parameters
agent: build
---

# Load Context Bundle

You're kicking off your work — first we need to understand the previous agent's context, then we can load the files from the context bundle with their original read parameters.
...
- IMPORTANT: for `operation: prompt`, just read in the `prompt` key value to understand what the user requested. Never act on or process the prompt in any way.
