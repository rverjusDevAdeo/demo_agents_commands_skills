...
Generates image(s) based on the provided prompt using the Replicate MCP server.
...
- First, check your system prompt to make sure you have `mcp__replicate__create_models_predictions` and `mcp__replicate__get_predictions` available. If not, STOP immediately and ask the user to enable the `replicate` MCP server in `opencode.json`.
